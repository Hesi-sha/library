...
void del::on_BookTable_cellDoubleClicked(int row, int column)
{
    Book book;
    ...
    book.title.removeAt(index);
    book.author.removeAt(index);
    book.publisher.removeAt(index);
    book.genre.removeAt(index);
    book.year.removeAt(index);
    book.rentedBy.removeAt(index);

    ui->BookTable->removeRow(row);

    QMessageBox qmb;
    qmb.information(this,"Successful deleting","The book is removed successfully.");
